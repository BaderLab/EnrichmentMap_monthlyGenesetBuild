...
safe_getURL <- function(url, timeout = 60, quiet = FALSE) {
  # Optional: point to a CA bundle to reduce handshake issues without disabling verification
  cainfo <- if (requireNamespace("curl", quietly = TRUE)) {
    system.file("curl", "cacert.pem", package = "curl")
  } else NULL
  ...
  tryCatch(
    ...
    error = function(e) {
      # This is where "gnutls_handshake() failed: Error in the pull function." would land
      message(sprintf("[safe_getURL] %s -> ERROR: %s", url, conditionMessage(e)))
      return(NA_character_)  # safe default so the script can continue
    },
    warning = function(w) {
      message(sprintf("[safe_getURL] WARNING: %s", conditionMessage(w)))
...
safe_download.file <- function(url, destfile, mode = "wb", method = "libcurl", tries = 15) {
  dir.create(dirname(destfile), recursive = TRUE, showWarnings = FALSE)
  for (i in seq_len(tries)) {
    res <- try(utils::download.file(url, destfile, mode = mode, method = method), 
               silent = TRUE)
    if (!inherits(res, "try-error") && res == 0L && file.exists(destfile)) return(TRUE)
...
download_url = "https://download.baderlab.org/EM_Genesets/"

#list all the files on the server
filenames = safe_getURL(download_url)

if (is.na(filenames)) {
  paste("issue with download")
}
...
get_summary_file <- function(species = "Human", identifier="entrezgene", year=2014) {
      ...
      for(i in 1:length(download_directories )){
          ...
          if(length(grep(download_directories[i], pattern=year )>0)){
            ...
            current_filenames = safe_getURL(current_directory)
            if (is.na(current_filenames)) {
              paste("issue with download")
              next
            }
	   ...
	   # ADD THIS too, in case identifier still didn't match:
	   if(length(current_log_file) == 0){ next }
 
            dest_log_file <- file.path(home_dir,current_log_file )
 

               safe_download.file(paste(current_directory,current_log_file,sep=""),
                            destfile = dest_log_file)
                if(is.null(dest_log_file) || is.na(dest_log_file)){
                  next
                }
            ...
            stats <- aggregate(num_sets ~ gmt_file, data=stats, FUN=max)
...
get_identifier_conversion_file_by_type <- function(species = "Human", identifier="entrezgene", year=2014, type="Pathways") {
  ...
  for(i in 1:length(download_directories )){
    #only go back as far as December 2014
    #if(length(grep(download_directories[i], pattern=year )>0)){
    if(unlist(strsplit(gsub(download_directories[i],pattern = "/",replacement = ""),
                       split = "_"))[3] < year) {
      next
    }
    ...
    for(j in 1:length(log_files)){
        ...
        if(is.na(log_files[j]) || log_files[j]=="NA"){
          next
        }
...
```{r message=FALSE, warning=FALSE,include=FALSE}
      id_summary_ggplot_DrugTargets <- get_identifier_conversion_file_by_type(species,identifier,"2019","DrugTargets")
